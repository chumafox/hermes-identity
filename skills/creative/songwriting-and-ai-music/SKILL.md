...
## 9. Suno Post-Generation Features

### Remaster (Audio Upscale)

Remaster — это AI-апскейл/полировка уже сгенерированного трека. Модель прогоняет аудио через второй проход, убирая артефакты генерации, поднимая детализацию и выравнивая частотный баланс.

**Три уровня:**

| Уровень | Эффект |
|---------|--------|
| **Subtle** | Минимальная обработка — чинит грубые артефакты (щелчки, зашумления). Характер трека почти не меняется. |
| **Normal** | Умеренный апскейл — поднимает битрейт/частоту дискретизации, чистит mid-range, добавляет чёткости вокалу и инструментам. Стандартный выбор. |
| **High** | Агрессивная реставрация — убирает почти все артефакты, но может "переполировать" звук, делая его стерильным или меняя тембр. |

**Когда что использовать:**
- **Subtle** — трек уже звучит хорошо, нужно убрать лёгкий шум
- **Normal** — стандартный случай, трек с артефактами средней тяжести
- **High** — сильно зашумлённый трек, готовность к изменению характера звука

**Важно:** Remaster не может исправить проблемы с произношением или текстом — это чисто аудио-обработка. Для исправления текста/произношения нужно перегенерировать трек.

### Другие пост-генерационные фичи Suno

- **Cover** — переосмысление трека в новом стиле (меняет жанр/аранжировку, сохраняя мелодию)
- **Extend** — продолжение трека от выбранной точки
- **Replace Section** — замена конкретного участка (секции) с новым промптом
- **Get Stems** — разделение на инструментальные дорожки (вокал, инструменты)
