...
5. **LM Studio HTTP 400**: model ID mismatch between Hermes config and LM Studio's loaded model. See `references/lm-studio-model-id-mismatch.md` for diagnostic steps.
6. **Local provider connection refused**: verify the local server (LM Studio, Ollama, llama.cpp) is running on the expected port. For LM Studio: `curl http://127.0.0.1:1234/v1/models`, for Ollama: `curl http://127.0.0.1:11434/api/tags`.
