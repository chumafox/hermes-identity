...
## Git Bundle Update (offline repo update)

When an existing git repo on the target Mac (e.g. `~/.hermes/hermes-agent`) needs updating but GitHub is blocked, use `git bundle` instead of trying to fix the tunnel:

### Pattern

```
SOURCE (with internet)         TARGET (no GitHub access)
─────────────────────          ─────────────────────────
1. cd ~/.hermes/hermes-agent
2. git bundle create
3. scp bundle to target
                               4. git fetch bundle
...
# Transfer:
scp -i ~/.ssh/key /tmp/hermes-update.bundle user@target-ip:/tmp/

# On target:
cd ~/.hermes/hermes-agent

# If local changes exist (common on pro — e.g. config tweaks):
git stash
...
- `hermes update` fails with `SSL_ERROR_SYSCALL` or `Couldn't connect to server`
- Git proxy is configured but the tunnel is dead (SSH master shows "Running" but port doesn't listen)
- Target Mac is in China without working VPN/proxy at the moment
- Any case where SCP/SSH works but HTTPS/GitHub is blocked

### Pitfalls

- **Check for local changes first** with `git status` before fetch+merge. The merge will abort with `Your local changes would be overwritten`. Always `git stash` first.
- **Bundle size** is typically ~5-7 MB for a week of commits on hermes-agent. `--since="7 days ago"` is a good default — adjust to `--since="30 days ago"` for infrequent updates.
- **Bundle covers full refs** — `HEAD` in the bundle includes everything reachable. The target must have a common ancestor, otherwise `git fetch bundle` pulls all history. For first-time setup, `--since` with a known ancestor tag works.
- **No compression needed** — git bundle is already compressed. SCP of a 5.7MB file takes ~1s over local WiFi.
